...
$teOtherService       = ($_POST['te_other_name']) ? $_POST['te_other_name'] : '';
...
$teaches       = ($_POST['teaches_name']) ? $_POST['teaches_name'] : '';
...
if (!empty($newImage)) {
    ...
    if ($_FILES["img_name"]["size"] <= 5000000) {
        if ($imageFileType == "jpg" || $imageFileType == "jpeg" || $imageFileType == "png") {
            ...
            $sql = "INSERT INTO tds(fname, mname, lname, mother_name, sex, type, birth_date, employment_period, reduced_service, other_service, te_other_service, education_level, teaches, department, minor, 3_major, teacher_level, retirement_number, region, zone, woreda, school, school_level, association, warranty, salary, shelf, kent, phone, image, warranter_jop, warranter_name, warranter_salary, warranter_photo, registered_by, yw, yw_full_name, yw_insti, yw_reason, credit, credit_type, place, place_date, password) 
            VALUES('$firstName', '$middleName', '$lastName', '$m_name', '$sex', '$type', '$birthDate', '$employmentPeriod', '$reduced_service', '$otherService', '$teOtherService', '$educationLevel', '$teaches', '$department','$minor', '$three_major', '$teacherLevel', '$retirementNumber', '$region', '$zone', '$woreda', '$school', '$school_level', '$association','$warranty', '$salary', '$shelf', '$kent', '$phone', '$ppname', '$warranterJop', '$warranterFullName', '$warranterSalary', '$wppname', '$registered_by', '$yw', '$yw_full', '$yw_inst', '$yw_re', '$cr_name', '$cr_type_name', '$pl_dr', '$place_day', '$teacherPassword')";
            ...
            if (strlen($teOtherService) == 8) {
                $int_te_otherService = explode("/", $teOtherService);
                $int_te_year = $int_te_otherService[0];
                $int_te_month = $int_te_otherService[1];
                $int_te_date = $int_te_otherService[2];

                $teOtherService = $int_te_month . '/' . $int_te_date . '/' . $int_te_year;
            }
            ...
            if (bdate_fun($b_date, $b_month, $b_year, $c_year) && edate_fun($b_year, $e_date, $e_month, $e_year, $ethiopian_c_date)) {
                if (preg_match($pattern2, $reduced_service) && strlen($reduced_service) == 8) {
                    if (preg_match($pattern2, $teOtherService) && strlen($teOtherService) == 8) {
