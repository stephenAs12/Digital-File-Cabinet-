...
$woreda         = (htmlspecialchars(stripslashes($_POST['woreda_name']))) ? htmlspecialchars(stripslashes($_POST['woreda_name']))  : '';
...
$check_woreda=false;
if ($userRole!="ወረዳ መርሱ ሀላፊ") {
    if ($woreda == '') {
        $woreda = 6;
        $check_woreda=true;
    }
}else if($userRole == "ወረዳ መርሱ ሀላፊ"){
    if ($woreda == '') {
        $check_woreda=false;
    }else{
        $check_woreda=true;
